...
When a contract is created, the Ethereum Virtual Machine executes any code in its constructor_ function (a function declared with the ``constructor`` keyword) once.
...
If you have a `public` state variable of array type, then it only returns individual elements to reduce the gas cost of returning an entire array. You can use parameters to specify which individual element to return, for example ``data[0]``. If you want to return an entire array in one call, then you need to write a helper function, for example::

    // public state variable
    uint[] public myArray;

    // caller function, generated by compiler
...
By adding the ``public`` keyword to the ``myArray`` variable, the ``getArray`` function behaves in a similar way to the function generated by the compiler.
...
A constructor is an optional function declared with the ``constructor`` keyword which is executed upon contract creation, and where you can run contract initialisation code. It always generates code that initialises any state variables you declare to their specified default value if you initialise them inline, or zero if you do not.

After that it returns the final code of the contract. The final deployment of the code costs additional gas linear to the length of the code, if you did not supply enough gas to initiate the state variables declared in the constructor, then an "out of gas" exception is generated. This is the **only** situation where an "out of gas" exception does not revert changes to the state.
